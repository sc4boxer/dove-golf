...
function EquipmentAlignmentShareCard({
  ...
  return (
    ...
    >
      ...
      <foreignObject x="88" y="636" width="904" height="154">
        <div
          xmlns="http://www.w3.org/1999/xhtml"
          className="grid h-full gap-6 [grid-template-columns:repeat(3,minmax(0,1fr))]"
        >
          {recommendationGroups.map((group, i) => (
            <div key={group.title} className={`min-w-0 ${i > 0 ? "border-l border-slate-200 pl-3" : ""}`}>
              <div className="text-base font-semibold text-slate-900">{group.title}</div>
              <div className="mt-2 space-y-2">
                {group.lines.slice(0, 6).map(([label, value]) => (
                  <div key={`${group.title}-${label}`} className="flex items-start gap-3 text-[13px] leading-[1.15rem]">
                    <span className="text-slate-500">{label}</span>
                    <span className="ml-auto min-w-0 break-words whitespace-normal text-right font-medium text-slate-900">{value}</span>
                  </div>
                ))}
              </div>
            </div>
