...
import { COURSE, createBall, strike, step, isMoving, type Ball } from "@/lib/putting/physics";
...
function draw(canvas: HTMLCanvasElement, ball: Ball, angle: number, power: number, phase: Phase) {
  ...
  ctx.fillStyle = "#365540"; ctx.font = "600 10px Arial"; ctx.textAlign = "center"; ctx.fillText("01", cup.x + 10, cup.y - 38);
  if (!ball.sunk) {
    if (phase === "ready") {
    ...
    const render = (time: number) => {
      ...
      if (!document.hidden && liveRound.current.phase === "rolling") {
        ...
        while (accumulated >= 1 / 120) {
          ball.current = step(ball.current, 1 / 120);
          accumulated -= 1 / 120;
        }
        if (!isMoving(ball.current)) {
          ...
          if (ball.current.sunk) setBest((value) => value === null ? next.strokes : Math.min(value, next.strokes));
            ...
            const nextAngle = aimAtCup(ball.current);
